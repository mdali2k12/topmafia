...
if($jobquery){
	$is=$conn->query("SELECT u.*,us.*,j.*,jr.*, f.*, vi.* FROM users_data u
	...
	$is=$conn->query("SELECT u.*,us.*,h.*, f.*, vi.* FROM users_data u 
	INNER JOIN users_stats us ON u.userid=us.userid 
	INNER JOIN users_vitals vi ON vi.userid=u.userid
	...
	$is=$conn->query("SELECT u.*,us.*, f.*, vi.* FROM users_data u 
	INNER JOIN users_stats us ON u.userid=us.userid 
	INNER JOIN users_finance f ON f.userid=u.userid
	INNER JOIN users_vitals vi ON vi.userid=u.userid
	INNER JOIN users_avatars a ON a.userid=u.userid
...
if ($_SESSION['token'] != $ir['sessiontoken']){
    session_unset();
	session_destroy();
    ...
    $sess = $k->fetch_assoc();
    if ($_SESSION['session'] != $sess['sesscode']){
    session_unset();
	session_destroy();
	    echo "<script type='text/javascript'>window.location.href = 'logout.php';</script>";
}
