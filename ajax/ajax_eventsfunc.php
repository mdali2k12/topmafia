...
$conn->query("DELETE FROM events WHERE evID={$id} AND evUSER=$userid");
        $data['success'] = "Event deleted!";
...
{
global $conn,$ir,$c,$userid,$h;
$q=$conn->query("SELECT * FROM events WHERE evUSER=$userid");
if(mysqli_num_rows($q)==0)
{
    $data['error'] = "Your events is already empty!";

    echo json_encode($data);
}
else {
$conn->query("DELETE FROM events WHERE evUSER=$userid");
        $data['success'] = "All events deleted successfully";
