...
if (!function_exists('check_level')) {
    function check_level()
    {
        ...
        if ($ir['exp'] >= $ir['exp_needed'] and $ir['level'] < 200) {
            ...
            $conn->query("UPDATE users_data SET level=level+1,exp=$expu,brave=brave+2,crystals=crystals+50,maxbrave=maxbrave+2,
    hp=hp+50,maxhp=maxhp+50,mlevel=mlevel+1 where userid=$userid");
            $conn->query("UPDATE users_data SET brave=maxbrave, energy=maxenergy, will=maxwill, hp=maxhp WHERE userid=$userid");
